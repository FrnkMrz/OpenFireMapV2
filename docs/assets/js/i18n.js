 ...
 * ==========================================================================================
 * DATEI: i18n.js
 * ZWECK: Sprach-Verwaltung (Wörterbuch & Logik)
 * BESCHREIBUNG:
 * Diese Datei besteht aus zwei Teilen:
 * 1. Dem Wörterbuch (deine alten Übersetzungen).
 * 2. Der Logik, die automatisch erkennt, welche Sprache der Nutzer spricht
 * und die Texte auf der Webseite austauscht.
 * ==========================================================================================
 */

// ==========================================================================================
// TEIL 1: DAS WÖRTERBUCH
// (Hier ist dein kompletter Inhalt aus der alten translations.js)
// ==========================================================================================
...
const translations = {
    ...
    // ... (deine weiteren Sprachen hier einfügen) ...
        // Aber dein Code funktioniert mit ALLEN Sprachen, die du hier einfügst.
};


// ==========================================================================================
// TEIL 2: DIE LOGIK (Das Gehirn)
// Hier wird ermittelt, welche Sprache der Besucher spricht.
// ==========================================================================================

/* 1. Sprache des Browsers ermitteln */
// Wir fragen den Browser: "Welche Sprache sprichst du?"
// Das Ergebnis ist z.B. "de-DE" (Deutsch-Deutschland) oder "zh-TW" (Chinesisch-Taiwan).
const userLangFull = navigator.language || navigator.userLanguage; 

// Wir holen uns den kurzen Code (die ersten 2 Buchstaben), z.B. "de" oder "en".
const userLangShort = userLangFull.split('-')[0].toLowerCase(); 

/* 2. Entscheidung treffen */
// Standard ist Englisch, falls wir die Sprache nicht kennen.
let currentLang = 'en';

// Spezial-Logik für Chinesisch (da gibt es Traditional vs. Simplified)
if (userLangFull.toLowerCase() === 'zh-tw') {
    currentLang = 'tw'; // Taiwan -> Traditionell
} else if (userLangFull.toLowerCase() === 'zh-hk') {
    currentLang = 'yue'; // Hongkong -> Kantonesisch
} else if (userLangShort === 'zh') {
    currentLang = 'zh'; // Rest -> Mandarin / Vereinfacht
} 
// Für alle anderen Sprachen (Deutsch, Französisch, Polnisch etc.):
// Wir schauen, ob wir den Code (z.B. 'pl') in unserem 'translations'-Objekt haben.
else if (translations[userLangShort]) {
    currentLang = userLangShort;
}

/**
 * FUNKTION: t(key) -> "Translate"
 * Holt den übersetzten Text für einen bestimmten Schlüsselbegriff.
 * * @param {string} key - Der Schlüsselbegriff (z.B. 'search_placeholder')
 * @returns {string} - Der fertige Text in der richtigen Sprache
 */
export function t(key) {
    // 1. Versuche, den Text in der aktuellen Sprache zu finden.
    // 2. Wenn nicht gefunden: Nimm Englisch als "Notnagel".
    // 3. Wenn auch das fehlt: Gib einfach den Schlüsselbegriff zurück (besser als nichts).
    return translations[currentLang]?.[key] || translations['en']?.[key] || key;
}

/**
 * FUNKTION: updatePageLanguage()
 * Geht durch die gesamte Webseite und tauscht alle Texte aus.
 * Sie sucht nach Elementen mit dem Attribut 'data-i18n'.
 */
export function updatePageLanguage() {
    // 1. Normale Texte austauschen (z.B. in <span> oder <div>)
    document.querySelectorAll('[data-i18n]').forEach(el => {
        const key = el.getAttribute('data-i18n');
        el.innerText = t(key);
    });

    // 2. Tooltips (Maus-Hinweise) austauschen (Attribut 'title')
    document.querySelectorAll('[data-i18n-title]').forEach(el => {
        const key = el.getAttribute('data-i18n-title');
        el.title = t(key);
    });

    // 3. Platzhalter in Eingabefeldern austauschen (Attribut 'placeholder')
    // z.B. im Suchfeld "Ort suchen..."
...
/**
 * FUNKTION: getLang()
 * Gibt den Code der aktuellen Sprache zurück (z.B. 'de', 'en').
 * Das brauchen wir später für die Datumsanzeige beim Export.
 */
